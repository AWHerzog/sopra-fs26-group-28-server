...
public class GameController {
	...
	@PostMapping("/games/{code}/lobby/leave")
	@ResponseStatus(HttpStatus.OK)
    public void leaveLobby(@PathVariable String code, @RequestHeader("Authorization") String token) {
		String username = userService.checkTokenAuthenticity(token).getUsername();
	...
	@PostMapping(value = "/games/{code}/lobby/leave/dirty", consumes = {"text/plain", "application/json", "*/*"})
	@ResponseStatus(HttpStatus.OK)
	public void dirty_leave_lobby(@PathVariable String code, @RequestParam String token){
		String username = userService.checkTokenAuthenticity(token).getUsername();
		gameService.leaveGame(code, username);
	}
	...
	@ResponseStatus(HttpStatus.OK)
	public GameStateGetDTO advance(@PathVariable String code, @RequestHeader("Authorization") String token){
	...
	public Map<String, String> translateQuestion(@PathVariable String code, @RequestParam String lang, @RequestHeader("Authorization") String token) {
		userService.checkTokenAuthenticity(token);
		String translated = gameFlowService.translateCurrentQuestion(code, lang);
